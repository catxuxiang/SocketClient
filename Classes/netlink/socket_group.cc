...
SocketGroup::SocketGroup(): _cmdOnAccept(nullptr), _cmdOnRead(nullptr), _cmdOnDisconnect(nullptr) {}
        ...
        while(launchSockets < status && i < _vSocket.size()) {
            ...
            if(FD_ISSET(_vSocket[i]->socketHandler(), &setSockets)) {
                ...
                if(_vSocket[i]->type() == SERVER && _vSocket[i]->protocol() == TCP) {
                    if(_cmdOnAccept != nullptr)
                        _cmdOnAccept(_vSocket[i], this, reference);
                } //if
                else {
                    if(_vSocket[i]->protocol() == TCP && !_vSocket[i]->nextReadSize()) {
                        if(_cmdOnDisconnect != nullptr)
                            _cmdOnDisconnect(_vSocket[i], this, reference);
                    }

                    else if(_cmdOnRead != nullptr)
                        _cmdOnRead(_vSocket[i], this, reference);
                }
